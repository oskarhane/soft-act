...
vorpalCLI
  .delimiter(chalk.green("[") + chalk.red("&~") + chalk.green("]"))
  .history("neo4j-keygen");

vorpalCLI
  .command("keys", "Show the current public/private keys being used")
...
vorpalCLI
  .command(
    "keys public [path]",
    "Set or get the public key used to verify signed messages."
  )
  .action(function(args, callback) {
    var self = this;
    if (args.path) {
...
vorpalCLI
  .command(
    "keys private [path]",
    "Set or get private key used to sign messages."
  )
  .action(function(args, callback) {
    var self = this;
    if (args.path) {
...
vorpalCLI
  .command("generate <codename>", "Create a software activation code.")
  .option("-f, --feature [name regex]", "Feature name to activate.")
  .option("-v, --version [semver range]", "Feature version range to activate.")
  .option("-u, --registrant [name regex]", "Full name of registrant.")
  .option("-o, --org [name regex]", "Organization of registrant.")
  .option("-e, --email [address regex]", "Email address of registrant.")
  .option("-d, --date [ISO 8601]", "Expiration date, in ISO 8601 format")
  .action(function(args, callback) {
    let self = this;
...
vorpalCLI
  .command("register <username>", "Mock a registered user.")
...
vorpalCLI
  .command("verify <codename>", "Verify a signed feature activation.")
  .action(function(args, callback) {
    var self = this;
    var activationCodeFilename = activationCodeFileFor(args.codename);
    const signedActivationCode = fs.readFileSync(
      activationCodeFilename,
      "utf8"
    );
    verifyCode(publicKeyPath, signedActivationCode).then(isValid => {
      self.log(
        isValid
          ? `${activationCodeFilename} is a ${chalk.green(
              "Valid Activation Code"
            )}`
          : `${activationCodeFilename} is ${chalk.red(
...
vorpalCLI
  .command(
    "can <username> <feature>",
    "According to available activation codes, can this registrant use this feature."
  )
  .action(function(args, callback) {
    let self = this;
        ...
        .then(can => {
          self.log(
            can
              ...
              : `${userInfo.registrant} ${chalk.red("can not")} ${
                  args.feature
                } `
          );
        })
...
vorpalCLI
  .command(
    "showcode <codename>",
    "Show the content of a feature activation code."
  )
  .action(function(args, callback) {
    var self = this;
    var activationCodeFilename = activationCodeFileFor(args.codename);
...
vorpalCLI
  .command("showuser <username>", "Show the user registration.")
  .action(function(args, callback) {
    var self = this;
    const registrationFilename = registrationFileFor(args.username);
